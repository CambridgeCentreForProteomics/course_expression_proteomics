...
## For this tutorial, we'll create example data since the actual data
...
assignments <- organelle_cor_wide %>%
  pivot_longer(cols = c(cytoplasm, nucleus, mitochondria, ER, golgi),
...
if (requireNamespace("Rtsne", quietly = TRUE)) {
  ...
  stop("Please install Rtsne package: install.packages('Rtsne')")
}
...
confidence_scores <- organelle_cor_wide %>%
  rowwise() %>%
  mutate(
    Max_Cor = max(c(cytoplasm, nucleus, mitochondria, ER, golgi)),
    Second_Cor = sort(c(cytoplasm, nucleus, mitochondria, ER, golgi), 
                      decreasing = TRUE)[2],
...
## Use tempdir() for a portable temporary directory
write.csv(localization_report, 
          file = file.path(tempdir(), "localization_results.csv"), 
          row.names = FALSE)
