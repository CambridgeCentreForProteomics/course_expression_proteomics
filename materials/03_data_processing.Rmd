...
library("enrichplot")
library("patchwork")
load("materials/output/lesson02.rda", verbose = TRUE)
```
...
This is one among several third party software available for protein
...
`QFeatures` object called `psms_filtered` so in the following code chunks we
always pass the argument `i = "psms_filtered"`.
...
Use the `filterFeatures` function filter out common contaminants that have 
...
We wish to aggregate from PSM to protein level so pass the argument `i =
"log_imputed_psms"` to specify we wish to aggregate the log transformed PSM
data, and then pass `fcol = "Master.Protein.Accessions"` to specify we wish to
...
::: {.callout-note}
#### Intermediate step of peptide aggregation
In this example we aggregate upwards from PSM to proteins. We could have 
included an intermediate step of aggregating from PSM to peptides, and then
peptides to proteins. To do this our first aggregation would have included
passing `fcol = "Sequence"` instead of `"Master.Protein.Accessions"`.
...
:::{.callout-exercise}
#### Challenge 5: Using NormalyzerDE
{{< level 1 >}}
...
on a dataset. The `normalyzer` function however requires a raw intenisty matrix
...
directly to protein level.

2. Run the `normalyzer` function on the newly created (untransformed) 
protein level data,
...
Run `normalyzer` note, we need to pass `requireReplicates = FALSE` as we have
only one sample of the control. For more details on using the `NormalyzerDE`
package take a look at the [package vignette](https://bioconductor.org/packages/release/bioc/vignettes/NormalyzerDE/inst/doc/vignette.html).
...
Let's verify the normalisation has worked in the next exercise and also by
viewing the `QFeatures` object. We can call `@ExperimentList` to view all
the assays we have created,
...
:::{.callout-exercise}
#### Challenge 6: Visualising the data prior and post-normalisation
{{< level 3 >}}

1. Create two boxplots pre- and post-normalisation to visualise the effect it has
had on the data.

2. Colour code each boxplot by condition.
...
```{r, include=FALSE}
save(cc_qf, file = "materials/output/lesson03.rda")
```
