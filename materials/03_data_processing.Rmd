...
* Generate key plots to visualise the processes of data cleaning, transformation and aggregation
...
Now that we have created a copy of our data, we can start the process of data 
...
regardless of experimental goal or data level (PSM or peptide). If a feature
...
Finally, depending upon the experimental question and goal (exploratory vs. 
stringent), we may also wish to remove features which:
...
MS3-based analysis can be used **add refs**. Nevertheless, we remove PSMs with a 
high percentage isolation interference to prevent the inclusion of inacurrate 
quantitation values.
...
1. Exploration of missing data - determine the number/proportion and pattern of missing values
...
we have a protein with three abundance values across samples A, B and C. If the
...
need to account for non-biological variance that may contribute to such differential
