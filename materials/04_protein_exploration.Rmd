...
support for protein groups. Let's do the intermediate step of aggreagting
to peptides before aggregating to PSMs.

:::{.callout-exercise}
#### Challenge 3: Peptide aggregation
{{< level 1 >}}

(i) Use the `aggregateFeatures` function to aggregate the logged and imputed PSMs to
peptides. Create a new `assay` in the your `QFeatures` data called
`"log_peptides"`.

(ii) Aggregate your new peptide assay to proteins and call this assay `"log_proteins_2"`.
