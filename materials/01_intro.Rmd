...
line (U20S) were treated with Nocodazole to inhibit the polymerisation of
...
In this approach the initial round of precursor fragmentation only uses a weak
fragmentation method (e.g., collision induced dissociation, CID) to ensure that
TMT reporter ions are not broken off at MS2. The most abundance fragment ions
...
MS3 level, rather than just one.
...
several that can be used to carry out a database search to identify peptide 
sequences, and therefore proteins, from raw MS data. Others include MaxQuant, 
FragPipe and PEAKS. 

Briefly, database searching of MS data involves comparing the observed MS spectra
to expected MS spectra generated via an in silico digestion of a selected protein 
...
between observed and expected spectra. 
