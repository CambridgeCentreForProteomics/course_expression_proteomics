...
We see we have created a new set called `peptides` and summarised 
...
We see we have now created a new set with `r nrow(cc_qf[["proteins"]])`
protein groups. 
...
We apply a center median normalisation, which shifts each sample's distribution
so that all sample medians are centered at zero.
...
`experiments` to view all the sets we have created,
