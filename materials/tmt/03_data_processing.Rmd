...
whole `QFeatures` object and all of its `SummarizedExperiment`s, or (ii) to specific
`SummarizedExperiment`s within the `QFeatures` object. We wish to only filter on the data in our
`QFeatures` object called `psms_filtered` (and not every dataset) so in the
