...
t-value or an F-value, respectively.Here, we are performing a single comparison, so we will obtain a t-value. 
...
Here, we will perform a comparison between two groups (M and G1 phases) for each protein and obtain a t-value and p-value for each protein. For a multifactorial comparison of all cell cycle stages, see [INSERT LINK HERE].
