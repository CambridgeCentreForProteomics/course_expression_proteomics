...
Depending on whether the linear model is used to perform single comparisons or multifactorial comparisons, the test statistic will either be a
t-value or an F-value, respectively.
...
A p-value may be obtained from a t/F-value by comparing the value against a t/F-distribution with the suitable degrees of freedom.
...
Here, we will perform a comparison between two groups (M and G1 phases) for each protein and obtain a t-value and p-value for each protein. For a multifactorial comparison, see [INSERT LINK HERE].
...
```{r}
## Identify the columns for the M/G1 samples in the protein assay
...
matrix should be created using `model.matrix(~variable)`, such that an intercept term is included. This means that the first level of the variable (here, M) is considered the 'baseline' and modeled by the intercept. The second level of the variable (here, G1) is then modelled by an additional term in the model, which captures the difference between M and G1. 

Below, we define the model matrix with an intercept term.
...
If one specified a model without an intercept for a single explanatory variable (
`model.matrix(~0 + variable)`), the resultant coefficients in the model will capture the difference between the group (M or G1) and zero. Given our null hypothesis relates to the difference *between the groups*, not *between each group and zero*, this is not want we want! We could use contrasts to explore the differences between the groups having not included an intercept term in our model, but it's simpler to work with a model which inherently estimates this difference.

::: callout-exercise
#### Challenge: Volcano plots

{{< level 1 >}} Inspect the design matrix obtained when not including an intercept term and contrast with the model including an intercept.

::: {.callout-answer collapse="true"}

```{r}

print(model.matrix(~0+condition)) # Model without an intercept
...
*t-value* (`t`) and *p-value* (`P.Value`). We also see an *adjusted
p-value* (`adj.P.Val`). This provides p-values adjusted to account
...
```{r, eval = FALSE}
ggsave(filename = "volcano_M_vs_G1.png", device = "png")
```
...
Thankfully, `limma` has in-built functions which allows us to specify a different null hypothesis and more appropriately test whether a protein has a fold-change greater than a given value. A statistical test whether the fold-change is greater than a specific value is more stringent than threshold on the point estimate and it thus makes sense to use a slightly lower threshold. Here we will use a threshold of absolute logFC > 0.5 (>1.4 fold-change).
...
# Need to now used TopTreat in place of TopTable
...
difference in abundance between M-phase and G1-phase cells.
...
```{r, include=FALSE}
## Save results
save(limma_results, all_proteins, file = here("materials/output/lesson06.rda"))
```
...
-   The statisticall appropriate way to threshold based on a log fold-change is
