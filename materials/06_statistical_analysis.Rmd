...
Next, we define the model matrix with an intercept term.
...
*t-value* (`F`) and *p-value* (`P.Value`). We also see an *adjusted
...
we would expect an average of 5 false positives.
...
proteins deemed differentially abundant will be false positives. It is up to you to decide what this threshold should be, but conventionally people between 0.01 (1%) and 0.1 (10%).
:::
...
consider is the `adj.P.Val` threshold that we wish to apply. The second parameter which is sometimes used to define significance is the `logFC`. This is mainly for
...
my_results <- 
  limma_results %>%
  mutate(significance = ifelse(adj.P.Val < 0.05 & abs(logFC)>1, "sig", "not.sig"))
