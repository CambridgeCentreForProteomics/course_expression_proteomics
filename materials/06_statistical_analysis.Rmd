...
Here, we will perform a comparison between two groups (M and G1 phases) for each protein and obtain a t-value and p-value for each protein. For a multifactorial comparison of all cell cycle stages, see [Statistical analysis of all 3 cell cycle stages](./08_statistical_analysis_all_stages.html).
...
#### Challenge: Design matrix without intercept

{{< level 1 >}} Inspect the design matrix obtained when not including an intercept term and contrast the models +/- intercept.
...
As expected, we see the two key parameters:

- *t-value* (`t`)
- *p-value* (`P.Value`). 

We also see an *adjusted p-value* (`adj.P.Val`). This provides p-values adjusted to account
...
- Compare the overall results of the logFC thresholding by tabulating the number of proteins with increased/decreased abundance and significant/not significant change. 
...
# Re-use the plot object with updated data using the '%+%' operator.
p2 <- p %+%
  mutate(highlight=Protein %in% sig_post_hoc_only, limma_results_treat)
print(p2)
