...
library("enrichplot")
library("patchwork")
load("output/lesson_1-4.rda", verbose = TRUE)
```
...
As expected, all of the PSMs in our data were of high confidence. 
...
the identification search), it is highly recommended to set a protein level 
FDR threshold. This is because each peptide can have multiple PSMs and each
protein can be supported by multiple peptides, thus resulting in the potential 
for amplification of the FDR during data aggregation. At a given PSM level score
...
about the protein level FDR in the PSM level output. Therefore, we have to import
the protein level output to get this information. The file is called
`cell_cycle_total_proteome_analysis_Proteins.txt` and we will load it in as a 
`tibble` using the tidyverse `read_delim` function.


```{r}
## Import protein level output from database search
protein_data <- read_delim(file = "data/cell_cycle_total_proteome_analysis_Proteins.txt")
```
...
The output contains `r dim(protein_data)` proteins. Let's compare this to the
...
We have a column called `Protein FDR Confidence Combined`. Since we are only
interested in the proteins that are found in our data, let's subset these from
the protein level output table. 

```{r}
## Subset proteins that are found in our filtered PSMs
our_proteins <- cc_qf[["psms_imputed"]] %>%
  rowData() %>%
  as_tibble() %>%
  pull(Master.Protein.Accessions) %>%
  unique()
...
protein_data <- protein_data %>%
  filter(Accession %in% our_proteins) %>%
  select(Accession, `Protein FDR Confidence Combined`)
```

Now we can print a table to see how many of the proteins in our data were found
...
Most of the proteins in our data were found to have high confidence (FDR < 0.01)
at protein level, but there are a few that are medium or low confidence. Even
though we set a PSM level FDR threshold of 0.01, some of the resulting proteins
still exceed this FDR due to amplification of error during aggregation. 

To remove the medium and low confidence proteins from our `psms_filtered` data,
we can add the corresponding protein confidence level to annotate each PSM in 
the `rowData`. This will then allow us to apply the `filterFeatures` function as
before.

```{r}
## Subset the accessions of proteins that correspond to each confidence level
high_conf <- protein_data %>%
  filter(`Protein FDR Confidence Combined` == "High") %>%
  pull(Accession)

med_conf <- protein_data %>%
  filter(`Protein FDR Confidence Combined` == "Medium") %>%
  pull(Accession)

low_conf <- protein_data %>%
  filter(`Protein FDR Confidence Combined` == "Low") %>%
  pull(Accession)


## Add this information to our rowData
rowData(cc_qf[["psms_imputed"]])$Protein_Confidence <- ifelse(rowData(cc_qf[["psms_imputed"]])$Master.Protein.Accessions %in% high_conf, "High", 
                                                               ifelse(rowData(cc_qf[["imputed"]])$Master.Protein.Accessions %in% med_conf, "Medium", "Low"))
```
...
```{r}
cc_qf <- cc_qf %>%
  filterFeatures(~ Protein_Confidence == "High", 
                 i = "psms_imputed")
```
...
```{r, include=FALSE}
save(cc_qf, file = "output/lesson_5.rda")
```
