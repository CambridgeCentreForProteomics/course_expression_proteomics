...
information to our `rowData` of the `psms_filtered` assay of our `QFeatures` 
object. 
...
```{r}
psm_data_QF <- cc_qf[["psms_filtered"]] %>% 
  rowData() %>% 
  as.data.frame()
```
...
cc_qf[["psms_filtered"]] %>%
  rowData() %>% 
  as_tibble() %>% 
  select(Protein.FDR.Confidence) %>%
  table()
```

Most of the PSMs in our data were found to originate from proteins with high confidence (FDR < 0.01,
...
telling us that `Protein.FDR.Confidence` was only found in 1 of our assays and 
as such no filter was applied to the raw data. This is correct and expected.
