...
First, let's examine what example DIA-NN data looks like. We'll simulate the
key structure of DIA-NN output to demonstrate the workflow:

```{r read_diann, eval=FALSE}
## Read DIA-NN main report
...
If you have your own DIA-NN output, replace "data/diann_report.tsv" with the path 
to your report file. DIA-NN outputs a tab-separated file by default.
:::
...
sample_metadata <- data.frame(
  ...
  fraction = c("Fraction1", "Fraction2", "Fraction3", 
               "Fraction4", "Fraction5", "Fraction6"),  # Example
  replicate = c(1, 1, 1, 1, 1, 1)  # Example
)
...
```{r filter_dia_specific, eval=FALSE}
...
We use `medianPolish` here as it is robust to outliers and handles the 
characteristics of DIA data well. Alternatives include `robustSummary` for 
data with missing values, or `colMeans`/`colMedians` for simpler aggregation.
:::


## Log transformation and normalisation

```{r transform_norm, eval=FALSE}
## Log2 transform
dia_qf <- logTransform(
  object = dia_qf,
  base = 2,
...
## Save the QFeatures object
save(dia_qf, file = here("course_files/preprocessed/lesson12.rda"))
```
