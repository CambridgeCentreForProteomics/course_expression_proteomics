...
between cell cycle stages is zero.

Our alternative hypothesis is: **(H1)** The change in abundance for a
protein between cell cycle stages is not zero.
...
t-value or an F-value, respectively. Here, we are performing a multifactorial comparison, so we will obtain an F-value. 
...
A p-value may be obtained from an F-value by comparing the value against an F-distribution with the appropriate degrees of freedom.
    
-   **degrees of freedom** = the number of observations minus the number
    of independent variables in the model
-   **p-value** = the probability of achieving the F-value under the
    null hypothesis i.e., by chance
    
Here, we will perform a comparison between three groups (M phase, G1 phase and
desynchronised) for each protein and obtain an F-value and p-value for each protein.

First, we extract the protein assay data. We will remove the pre-treatment sample,
since this condition is not replicated and so not amenable to statistical testing.
```{r}

## Identify the columns for the Pre-treatment sample in the protein assay
pre_treatment_col_ix <- colData(cc_qf[["log_norm_proteins"]])$condition == 'Pre-treatment'

# Extract the protein assay from our QFeatures object, excluding the pre-treatment sample
all_proteins <- cc_qf[["log_norm_proteins"]][, !pre_treatment_col_ix]
...
```{r, include=FALSE}
## Save results
save(limma_results_all_contrasts, all_proteins, file = here("materials/output/lesson08.rda"))
```
