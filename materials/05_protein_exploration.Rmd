...
was not present in the PSM level assays.
...
The output tells us that we have `r .support[1]` proteins with `r
names(.support[1])` peptides, `r .support[2]` proteins with support from 
`r names(.support[2])` peptides, and so forth.
...
plot to read if the aim is to get an overview of how many peptides are available per 
protein group. 
