...
M_G1_results$protein_description <- all_proteins %>%
  rowData() %>%
  as_tibble() %>%
  pull(Master.Protein.Descriptions)

## Extract proteins that have significant abundance changes in M vs. G1
sig_changing <- M_G1_results %>% 
  as_tibble() %>%
...
```{r}
## Look at descriptions of proteins upregulated in G1 relative to M
sig_up %>%
...
upregulated in G1 phase are annotated with the GO term "kinase activity", it may
seem intuitive to conclude that this biological process is important for the G1 
