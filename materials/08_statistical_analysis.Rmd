...
```{r}
## Format results
limma_results <- topTable(fit = fit_contrasts,      
...
different hypothesis. Here we have carried out `r nrow(limma_results)`
...
As we saw above, `topTable` will give us the output of our ANOVA model. We 
previously used this function to generate our `limma_results` without specifying
...
When we do not pass a `coef` argument the `topTable` function carries out an 
ANOVA test in which the null hypothesis is that each individual null hypothesis
is true. In other words, the null hypothesis for each protein is that the 
mean abundance in M-phase = the mean abundance in G1-phase = the mean abundance
in desynchronised cells. The three pairwise comparisons are all combined into
a single *F*-test resulting in an ANOVA output for each protein. This ANOVA test
not only involves multiple hypothesis testing down the proteins but also across
the contrasts for a protein. 
...
From this output we can see that some of our proteins have high F-values and 
...
```{r}
M_Desynch_results <- topTable(fit = fit_contrasts, coef = "M_Des", 
...
Interpreting the output of `topTable` for a contrast:
...
This is because we only told the function to compare two conditions, so a t-test
was carried out rather than an ANOVA test. Importantly, however, when a t-test
is completed there is no additional adjustment made for the multiple contrasts
of each protein. 
...
```{r}
decideTests(fit = fit_contrasts, adjust.method = "BH", 
            p.value = 0.01, method = "separate") %>%
  summary()
```
...
get the results of the `"M_Des"` contrast. We use the argument `confint = TRUE`
so that the our output reports the 95% confidence interval of the calculated 
log2FC. Whilst 95% is the default interval, a numeric value between 0 and 1 can 
also be passed if an alternative confidence interval is required.
