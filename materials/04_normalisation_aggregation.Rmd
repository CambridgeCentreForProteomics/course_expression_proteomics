...
Logarithmic transformation can also be applied at any stage of the data
processing. We have chosen to log transform the data in this example prior to
protein aggregation as we will use the `robust` method for summarisation and
this method requires the data to be log transformed.
...
1. Create a copy of your `cc_qf` dataset for testing normalisation methods. Let's
call this `norm_qf`.
...
Note: To run `normalyzer` on this data we need to pass `requireReplicates = FALSE` 
as we have only one sample of the control. For more details on using the
