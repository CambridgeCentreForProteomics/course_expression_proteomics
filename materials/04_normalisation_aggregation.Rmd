...
under `outputs` called `normalyzer`. Take a look at the PDF report. What method
...
peptides. This time take the logged PSM data and aggregate upwards straight to
the protein level. Give the newly created data level the name
"log_proteins_direct".
...
from PSMs using the median method.
...
```{r}
pre_norm <- cc_qf[["log_proteins_direct"]] %>%
  assay() %>%
  longFormat() %>%
