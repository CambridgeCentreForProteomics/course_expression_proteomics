...
We begin by extracting the normalised protein level data `"norm_protiens"` in
the QF object. This contains `r `nrow(dia_qf[["norm_proteins"]])` proteins common
...
filter, and link it to the parent set. We see we are left with 
`r table(tokeep)["TRUE"]` proteins to test.
